...
try:
    ...
    print(f"⚠️ API Config Warning: {e}")
...
def dashboard():
    ...
    stats = {
        "total_impressions": f"{total_impressions:,}", "engagement_rate": "5.2%",
        "link_clicks": f"{total_clicks:,}", "active_campaigns": 3,
        "trend_labels": [], "trend_data": [], "li_pct": 33, "fb_pct": 33, "ig_pct": 34
    }
    return render_template('dashboard.html', posts=posts, stats=stats)

@app.route('/api/submit_booking', methods=['POST'])
def submit_booking():
    if send_lead_email(request.form.get('name'), request.form.get('phone'), request.form.get('date'), request.form.get('message')):
...
def confirm_post():
    ...
    if action == 'instant':
        ...
        status = "Published" if "✅" in str(results) else "Failed"
    else:
